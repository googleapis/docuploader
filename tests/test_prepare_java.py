...
class PrepareDocsTest(unittest.TestCase):
  ...
  def setUp(cls):
    ...
    assert (filecmp.cmp(mock_original_toc, mock_updated_toc, shallow=False)) is False
    ...
    prepare_java_toc(mock_original_toc, "google-cloud-testing")
  ...
  def test_original_not_copy(self):
    assert (filecmp.cmp(mock_original_toc, mock_original_toc_copy, shallow=False)) is False
